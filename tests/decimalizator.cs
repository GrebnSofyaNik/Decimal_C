...
class Decimalizator
{
  ...
  public static void s21_div_mod()
  {
    Console.WriteLine("TEST: ЕБУЧИЙ СЛУЧАЙ");
    int itr = 1;
    BinaryReader reader = new BinaryReader(File.Open("s21_div.result", FileMode.Open));
    while (reader.BaseStream.Position != reader.BaseStream.Length)
    {
      decimal op1 = reader.ReadDecimal();
      decimal op2 = reader.ReadDecimal();
      decimal result = reader.ReadDecimal();
      try
      {
        if (op1 / op2 != result)
        {
          try { Console.WriteLine("{4}. in s21_mod: {0} % {1} = {2} ({3})", op1, op2, result, op1 % op2, itr++); }
          catch (Exception e) { Console.WriteLine("{3}. in s21_mod: {0} % {1} owerflowed", op1, op2, result, itr++); }
        }

      }
      catch (System.Exception)
      {
        Console.WriteLine("{3}. in s21_mod: EXCEPTION: {0} % {1} = {2}", op1, op2, result, itr++);
      }
    ...
    Console.WriteLine();
    itr = 1;
    reader = new BinaryReader(File.Open("s21_mod.result", FileMode.Open));
    while (reader.BaseStream.Position != reader.BaseStream.Length)
    {
      decimal op1 = reader.ReadDecimal();
      decimal op2 = reader.ReadDecimal();
  ...
  public static void s21_div()
  {
    ...
    BinaryReader reader_input = new BinaryReader(File.Open(testname + ".input", FileMode.Open));
    BinaryReader reader_output = new BinaryReader(File.Open(testname + ".output", FileMode.Open));
    BinaryWriter writer = new BinaryWriter(File.Open(testname + ".err", FileMode.Create));
